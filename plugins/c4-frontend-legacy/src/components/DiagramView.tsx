...
function DiagramViewInner({
  ...
  const { descriptors, loading, building } = useEntityC4Views(
    kind,
    namespace,
    name,
  );
  ...
  const selectedViewId =
    viewId ||
    (descriptors && descriptors.length > 0 ? descriptors[0].id : undefined);
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          {descriptors.map(d => (
            <button
              ...
              onClick={() =>
                navigate(
                  `/c4/${namespace}/${kind}/${name}/${encodeURIComponent(
                    d.id,
                  )}`,
                )
              }
              style={{
                fontWeight: d.id === selectedViewId ? 600 : 'normal',
                background: 'none',
