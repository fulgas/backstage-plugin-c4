...
const sampleDiagram: C4Diagram = {
  ...
  nodes: [{ id: 'domain:default/my-domain', depth: 0, name: 'My Domain', description: '', tags: [] }],
  actors: [],
  relationships: [],
};
...
describe('GET /views', () => {
  ...
  it('returns descriptors from store', async () => {
    const app = express();
    app.use(await createRouter({ store: mockStore({ getViewDescriptors: jest.fn().mockResolvedValue([sampleDescriptor]) }), syncFn: jest.fn() }));
    const res = await request(app).get('/views');
...
describe('GET /views/:id', () => {
  ...
  it('returns diagram when found', async () => {
    const app = express();
    app.use(await createRouter({ store: mockStore({ computeDiagram: jest.fn().mockResolvedValue(sampleDiagram) }), syncFn: jest.fn() }));
...
describe('GET /entity/:kind/:namespace/:name/views', () => {
  ...
  it('returns descriptors for entity', async () => {
    const getViewDescriptors = jest.fn()
      .mockResolvedValueOnce([sampleDescriptor])   // allDescriptors check
      .mockResolvedValueOnce([sampleDescriptor]);   // entity-specific
    const app = express();
    app.use(await createRouter({ store: mockStore({ getViewDescriptors }), syncFn: jest.fn() }));
    const res = await request(app).get('/entity/domain/default/my-domain/views');
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(1);
  });
});
