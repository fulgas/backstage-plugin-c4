...
export class CatalogProcessor {
  ...
  async process(): Promise<{
    ...
    const apiMap = new Map<
      string,
      { name: string; providers: Set<string>; consumers: Set<string> }
    >();

    // Pass 0: build domain parent chain (for spec.subdomainOf support)
    const domainToParent = new Map<string, string>();
    for (const entity of entities) {
      if (entity.kind === 'Domain') {
        const ref = stringifyEntityRef(entity);
        const parentRef = (entity.relations ?? []).find(
          r => r.type === 'partOf' && r.targetRef.startsWith('domain:'),
        )?.targetRef;
        if (parentRef) domainToParent.set(ref, parentRef);
      }
    }
    ...
    for (const entity of entities) {
      ...
      if (entity.kind === 'Domain') {
        const parentDomainRef = domainToParent.get(ref);
        const node: C4Node = {
          id: ref,
          parentId: parentDomainRef,
          depth: 0,
          name: entity.metadata.name,
          description: (entity.metadata.description as string) ?? '',
          navigable: true,
        ...
        descriptors.push({
          id: `catalog-landscape-${sanitize(ref)}`,
      ...
      if (entity.kind === 'System') {
        const domainRef = (entity.relations ?? []).find(
          r => r.type === 'partOf' && r.targetRef.startsWith('domain:'),
        )?.targetRef;
        if (domainRef)
          systemToDomain.set(ref, resolveRootDomain(domainRef, domainToParent));
        const node: C4Node = {
          id: ref,
          parentId: domainRef,
          depth: 1,
        ...
        nodeMap.set(ref, node);
        descriptors.push({
          id: `catalog-context-${sanitize(ref)}`,
          title: entity.metadata.name,
          subjectId: ref,
          entityRef: ref,
          source: 'catalog',
      ...
      if (entity.kind === 'Component' || entity.kind === 'Resource') {
        const systemRef = (entity.relations ?? []).find(
          r => r.type === 'partOf' && r.targetRef.startsWith('system:'),
        )?.targetRef;
        const componentParentRef =
          entity.kind === 'Component'
            ? (entity.relations ?? []).find(
                r =>
                  (r.type === 'childOf' || r.type === 'partOf') &&
                  r.targetRef.startsWith('component:'),
              )?.targetRef
            : undefined;
        const isSubcomponent = !!componentParentRef;
        const subType =
          entity.kind === 'Resource'
            ? resourceSubType(entity.spec?.type as string)
            : 'service';
        const node: C4Node = {
          id: ref,
          parentId: isSubcomponent ? componentParentRef : systemRef,
          depth: isSubcomponent ? 3 : 2,
          name: entity.metadata.name,
          description: (entity.metadata.description as string) ?? '',
          technology: (entity.spec?.type as string) ?? undefined,
          subType,
          navigable: entity.kind === 'Component' && !isSubcomponent,
          tags: (entity.metadata.tags as string[]) ?? [],
          catalogEntityRef: ref,
        };
        nodes.push(node);
        nodeMap.set(ref, node);
        // Resources and subcomponents are leaf nodes — no own diagram.
        if (entity.kind === 'Component' && !isSubcomponent) {
          descriptors.push({
            id: `catalog-container-${sanitize(ref)}`,
            title: entity.metadata.name,
            subjectId: ref,
            entityRef: ref,
            source: 'catalog',
        ...
        for (const relation of entity.relations ?? []) {
          if (
            ...
            !relation.targetRef.startsWith('api:')
          ) {
            relationships.push({
              id: uuidv4(),
              sourceId: ref,
    ...
    // Roll up depth-3→depth-3 cross-component to depth-2→depth-2
    const compRelSeen = new Set<string>();
    const compRelationships: C4Relationship[] = [];
    for (const r of relationships) {
      ...
      if (
        ...
        if (!compRelSeen.has(key)) {
          compRelSeen.add(key);
          compRelationships.push({
            id: uuidv4(),
    ...
    for (const r of [...relationships, ...compRelationships]) {
      const srcNode = nodeMap.get(r.sourceId);
      const tgtNode = nodeMap.get(r.targetId);
      if (
        srcNode?.depth === 2 &&
...
function resolveRootDomain(
  domainRef: string,
  domainToParent: Map<string, string>,
): string {
  let current = domainRef;
  const visited = new Set<string>();
  while (domainToParent.has(current) && !visited.has(current)) {
    visited.add(current);
    current = domainToParent.get(current)!;
  }
  return current;
}
